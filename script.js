class MemoryGame {
    constructor() {
        ...
        if (!window.GAME_CONFIG) {
            console.error('GAME_CONFIG не найден!');
            return;
        }
        
        this.config = window.GAME_CONFIG;
        this.metricaId = this.config.analytics.yandexMetricaId;
        ...
        // Пути к изображениям карточек из конфига
        this.cardValues = this.config.images.cards;
    ...
    showWinPopup() {
        ...
        this.sendMetric('game_completed_promo_shown', {
            ...
            promoCode: this.config.game.promoCode
        });
    }
    ...
    // Универсальный метод для отправки метрик - использует ID из конфига
    sendMetric(name, params = {}) {
        if (window.ym && typeof window.ym === 'function' && this.metricaId) {
            try {
                window.ym(this.metricaId, 'reachGoal', name, params);
            } catch (e) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (window.ym && typeof window.ym === 'function' && window.GAME_CONFIG) {
        const metricaId = window.GAME_CONFIG.analytics.yandexMetricaId;
        window.ym(metricaId, 'reachGoal', isMobile ? 'game_mobile' : 'game_desktop');
    }
});